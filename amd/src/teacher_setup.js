define(['core/ajax'], function(Ajax) {
    ...
    // Plantilla por defecto al seleccionar una actividad VPL sin configuración previa:
    // un único criterio vacío con los tres niveles base. Los niveles se reutilizan en
    // addCriterion para mantener el mismo punto de partida cuando el profesor añade
    // criterios nuevos.
    const buildDefaultLevels = () => [
        {id: createId('level'), name: 'Excelente', percentage: 100, description: ''},
        {id: createId('level'), name: 'Regular', percentage: 50, description: ''},
        {id: createId('level'), name: 'Insuficiente', percentage: 0, description: ''}
    ];

    const buildDefaultRubric = () => [{
    ...
    const createState = () => ({
        ...
        selectedVPL: '',
        config: null,
        rubricCriteria: buildDefaultRubric(),
        editablePrompt: '',
        promptDirty: false,
        promptLocked: true,
        students: [],
        ...
        bulkRunning: false,
        activeResult: null,
        resultDraft: {criteria: [], finalfeedback: '', studentfeedback: ''},
        savingResult: false,
        resultsResetNotice: '',
        // Instantánea de la rúbrica y de las referencias manuales guardadas, para
        // detectar cambios críticos antes de "guardar y continuar a resultados".
        rubricSnapshot: '',
        manualsSnapshot: '',
    });
    ...
    const applyState = data => {
        ...
        if (data.criteria && data.criteria.length) {
            ...
            state.rubricCriteria = buildDefaultRubric();
        }
    ...
    // Firma de los elementos que, si cambian, invalidan las evaluaciones IA previas:
    // criterios, niveles y calificaciones manuales de referencia.
    const rubricSignature = () => JSON.stringify((state.rubricCriteria || []).map(criterion => ({
        n: criterion.name,
        w: Number(criterion.weight) || 0,
        l: (criterion.levels || []).map(level => ({n: level.name, p: Number(level.percentage) || 0})),
    })));

    const manualsSignature = () => JSON.stringify((state.manuals || []).map(manual => ({
        s: manual.studentid,
        sub: manual.submissionid,
        c: (manual.criteria || []).map(item => ({c: item.criterionid, l: item.levelid})),
    })));

    const captureConfigSnapshot = () => {
        state.rubricSnapshot = rubricSignature();
        state.manualsSnapshot = manualsSignature();
    };

    // ¿Cambió algo crítico (criterios/niveles/manuales) frente a lo último guardado?
    const criticalConfigChanged = () => rubricSignature() !== state.rubricSnapshot
        || manualsSignature() !== state.manualsSnapshot;
            ...
            <div class="ag-stack">
                <div class="ag-field">
                    ...
                    ${state.selectedActivity ? `
                        <p class="ag-success-line">
                            Actividad seleccionada · ${state.students.length} estudiante(s) con entregas.
                            ${state.config ? 'Configuración cargada desde Moodle.' : 'Todavía no tiene configuración guardada.'}
                        </p>
                    ` : ''}
                </div>
            </div>
    ...
    const renderRubricCard = () => {
        ...
        region('rubric-card').innerHTML = `
            ...
            <div class="ag-add-criterion-row">
                <button type="button" class="ag-btn ag-btn--outline ag-btn--sm ag-btn--full-mobile" data-action="add-criterion"
            ...
            <div class="ag-total ${total === 100 ? 'is-ok' : 'is-error'}">
                Total: ${formatNumber(total)}% ${total !== 100 ? '(todos los criterios deben sumar 100%)' : ''}
            </div>
        `;
    };
    ...
    const renderLevel = (criterion, level) => `
        <div class="ag-level-row" data-level-id="${escapeAttr(level.id)}">
            ...
            <textarea class="ag-textarea ag-textarea--sm ag-level-description" rows="2"
                placeholder="Describe cuándo aplica este nivel (la IA usará este texto para calibrar)"
                data-action="level-field" data-field="description"
                data-criterion-id="${escapeAttr(criterion.id)}" data-level-id="${escapeAttr(level.id)}">${escapeHtml(level.description)}</textarea>
        </div>
    `;
    ...
    const renderAiResults = () => {
        ...
        return `
            <div class="ag-ai-results">
                <header class="ag-ai-header" style="${totalStyle}">
                    <div class="ag-ai-header-info">
                        <strong>Evaluación completada y guardada</strong>
                        <span>${escapeHtml(result.studentName)}</span>
                    </div>
                    <div class="ag-ai-header-score">
                        <strong>${formatNumber(total)}</strong>
                        <span>/100</span>
                    </div>
                </header>

                <section class="ag-breakdown-card">
                    <p>Desglose por criterio</p>
                    ${details.map(item => `
                        <div class="ag-breakdown-row" style="${gradeStyle(item.score, item.max)}">
                            <div class="ag-breakdown-row-head">
                                <div>
                            ...
                            ${(item.detail || '').trim() !== '' ? `
                                <p class="ag-breakdown-detail">${escapeHtml(item.detail)}</p>
                            ` : ''}
                        </div>
                    `).join('')}
                </section>

                <section class="ag-feedback-box">
            ...
            ${state.aiTests.length ? `
                <div class="ag-history-list">
                    ${state.aiTests.map(item => `
                        <div class="ag-history-item ${state.latestAiResult && String(state.latestAiResult.id) === String(item.id) ? 'is-editing' : ''}">
                            <div>
                                <strong>${escapeHtml(item.studentName)}</strong>
                                <span>Entrega #${item.submissionid} · ${escapeHtml(item.timereceivedText)} · ${formatNumber(item.totalgrade)}/100</span>
                            </div>
                            <div class="ag-history-actions">
                                <button type="button" class="ag-btn ag-btn--ghost ag-btn--xs" data-action="view-ai-test"
                                    data-test-id="${item.id}">Ver</button>
    ...
    const renderResultsContext = () => {
        ...
        const pendingTotal = (summary.pending || 0) + (summary.processing || 0);
        return `
            <div class="ag-results-heading">
                <div>
                    <h3 class="ag-card-title">Resultados de evaluación IA</h3>
                    <p class="ag-muted">Revisa, ajusta y publica la calificación y retroalimentación generada por IA para entregas VPL.</p>
                </div>
            ...
            ${state.resultsResetNotice ? `
                <div class="ag-info-box ag-info-box--yellow ag-reset-notice">
                    <span>${escapeHtml(state.resultsResetNotice)}</span>
                    <button type="button" class="ag-link-btn" data-action="dismiss-reset-notice">Entendido</button>
                </div>
            ` : ''}
            <div class="ag-results-context">
                ...
                <div class="ag-result-metrics">
                    ${resultMetric('Total', summary.total || 0, 'neutral')}
                    ${resultMetric('Evaluados', summary.evaluated || 0, 'green')}
                    ${resultMetric('Pendientes', pendingTotal, 'amber')}
                    ${resultMetric('Publicados', summary.published || 0, 'blue')}
                </div>
            </div>
        `;
    };

    const filtersAreActive = () => state.resultSearchTerm !== ''
        || state.resultStatusFilter !== 'all'
        || state.publicationFilter !== 'all';

    // El bloque de filtros y la lista se renderizan por separado: así, al
    // escribir en el buscador solo se actualiza la lista (data-region
    // "results-list") y el <input> nunca se reemplaza, conservando el foco.
    const renderResultsTable = () => `
        ${renderResultsFilters()}
        <div data-region="results-list">${renderResultsList()}</div>
    `;

    const renderResultsFilters = () => `
        <div class="ag-filters-block">
            <h3 class="ag-card-subtitle">Filtrar resultados</h3>
            <div class="ag-search-field">
                <span class="ag-search-icon" aria-hidden="true"></span>
                <input class="ag-input" type="text" placeholder="Buscar estudiante… (nombre / usuario)"
            ...
            <div class="ag-result-filters">
                <div class="ag-field">
                    ...
                    <select class="ag-select" data-action="result-status-filter">
                        <option value="all" ${state.resultStatusFilter === 'all' ? 'selected' : ''}>Todos</option>
                ...
                <div class="ag-field">
                    ...
                    <select class="ag-select" data-action="publication-filter">
                        <option value="all" ${state.publicationFilter === 'all' ? 'selected' : ''}>Todos</option>
            ...
            <button type="button" class="ag-btn ag-btn--outline ag-btn--sm" data-action="clear-result-filters"
                ${filtersAreActive() ? '' : 'disabled'}>
                Limpiar filtros
            </button>
        </div>
    `;
    ...
    const renderResultsList = () => {
        ...
        return `
            <div class="ag-results-toolbar">
                <div>
                    <h3 class="ag-card-subtitle">Resultados</h3>
            ...
            ${filtered.length ? `
                <div class="ag-results-table-wrap">
                    <table class="ag-results-table">
                        <thead>
                            <tr>
                                <th class="ag-col-check">
                                    <input type="checkbox" data-action="toggle-all-results"
                                        ${state.selectedResults.length === filtered.length && filtered.length ? 'checked' : ''}>
                                </th>
                                <th>Estudiante y fecha de entrega</th>
                                <th class="ag-col-feedback">Feedback IA (vista previa)</th>
                                <th>Estado IA</th>
                                <th>Nota IA</th>
                                <th>Nota final</th>
                                <th>Publicación</th>
                                <th class="ag-col-actions">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            ` : `
                <div class="ag-empty">
                    <span class="ag-empty-icon" aria-hidden="true"></span>
                    <h3 class="ag-empty-title">No hay resultados</h3>
            ...
            ${state.selectedResults.length ? `
                <div class="ag-batch-bar">
                    <div class="ag-batch-bar-info">
                        <span><strong>${state.selectedResults.length}</strong> estudiante(s) seleccionado(s)</span>
                        <p class="ag-muted">Aprobar y publicar muestra la nota y feedback al estudiante.</p>
                    </div>
                    <div class="ag-batch-bar-actions">
                        <button type="button" class="ag-btn ag-btn--primary ag-btn--sm" data-action="publish-selected-results"
                            ...
                            ${iconCheck()}
                            Aprobar y publicar (${selectedPublishableResults().length})
                        </button>
    ...
    // Actualiza solo la lista de resultados (no el bloque de filtros), para no
    // reemplazar el campo de búsqueda mientras el docente escribe.
    const refreshResultsList = () => {
        const listRegion = region('results-list');
        if (listRegion) {
            listRegion.innerHTML = renderResultsList();
        }
        const clearBtn = root.querySelector('.ag-filters-block [data-action="clear-result-filters"]');
        if (clearBtn) {
            clearBtn.disabled = !filtersAreActive();
        }
    };

    const renderResultRow = result => {
        const hasFeedback = Boolean((result.finalfeedback || resultAiFeedback(result) || '').trim());
        return `
        <tr>
            <td class="ag-col-check">
            ...
            <td class="ag-col-feedback">
                ...
                ` : '<span class="ag-feedback-empty">Sin feedback disponible</span>'}
            </td>
            <td>
                <div class="ag-status-cell">
                    ${resultStatusBadge(result.aistatus)}
                    ${result.aistatus === 'error' ? `
                        <button type="button" class="ag-link-btn" data-action="view-result" data-result-id="${result.id}">Ver detalle</button>
                    ` : ''}
                </div>
            </td>
            ...
            <td>
                <div class="ag-pub-cell">
                    ${publicationBadge(result.publicationStatus)}
                    ${reviewBadge(result)}
                </div>
            </td>
    ...
    const renderResultDrawer = () => {
        ...
        const activity = state.resultsState ? state.resultsState.activity : state.selectedActivity;
        const evaluated = result.aistatus === 'evaluated' && result.aitotalgrade !== null;
        // El nombre y la descripción del ejercicio se toman del propio resultado (los adjunta
        // el backend en cada petición); si por algún motivo no vinieran, se cae a la actividad.
        const exerciseName = (result && result.activityName)
            ? result.activityName
            : (activity && activity.name ? activity.name : '');
        const exerciseDescription = (result && result.activityDescription)
        ...
        region('main-action').innerHTML = `
            ...
            <aside class="ag-result-drawer ag-result-drawer--wide" role="dialog" aria-modal="true" aria-label="Resultado de ${escapeAttr(result.studentName)}">
                <div class="ag-drawer-header">
                    <div class="ag-drawer-identity">
                        <h3>${escapeHtml(result.studentName)} <span class="ag-drawer-username">@${escapeHtml(result.studentUsername)}</span></h3>
                        <div class="ag-drawer-meta">
                            <span>${escapeHtml(activity ? activity.name : 'Actividad VPL')}</span>
                            <span>·</span>
                            <span>Intento #${result.attemptnumber} · ${escapeHtml(result.timesubmittedText)}</span>
                        </div>
                    </div>
                    <button type="button" class="ag-icon-btn" data-action="close-result-drawer" aria-label="Cerrar">${iconClose()}</button>
                </div>

                ${renderDrawerExercise(exerciseName, exerciseDescription)}

                ${renderPhaseDiagram(result)}
                ...
                <div class="ag-drawer-footer">
                    <div class="ag-drawer-footer-left">
                        <button type="button" class="ag-btn ag-btn--outline ag-btn--sm" data-action="close-result-drawer">Cerrar</button>
                        <button type="button" class="ag-btn ag-btn--sm" data-action="save-result-review"
                            ${state.savingResult || !evaluated ? 'disabled' : ''}>${state.savingResult ? 'Guardando...' : 'Guardar revisión'}</button>
                    </div>
                    <button type="button" class="ag-btn ag-btn--primary ag-btn--sm" data-action="publish-result" data-result-id="${result.id}"
                        ${isPublishable(result) ? '' : 'disabled'}>
                        ${iconSend()}
                        Aprobar y publicar
                    </button>
                </div>
                ${!evaluated ? `
                    <div class="ag-drawer-hint">Evalúa la entrega con IA para poder revisar y publicar.</div>
                ` : ''}
            </aside>
        `;
    };

    // Diagrama de fases del resultado, en forma de flechas encadenadas (proceso por
    // fases). Las tres primeras fases también se muestran al estudiante en su asistente.
    const renderPhaseDiagram = result => {
        const graded = result.aistatus === 'evaluated' && result.aitotalgrade !== null;
        const reviewed = Boolean(result.reviewedByTeacher);
        const modified = Boolean(result.modifiedByTeacher);
        const published = result.publicationStatus === 'published';
        const phases = [
            {key: 'ai', label: 'Calificado por IA', icon: iconRobot(), done: graded},
            {key: 'reviewed', label: 'Revisado por docente', icon: iconEye(), done: reviewed},
            {key: 'modified', label: 'Modificado por docente', icon: iconPencil(), done: modified, optional: true},
            {key: 'published', label: 'Publicado', icon: iconSend(), done: published},
        ];
        return `
            <ol class="ag-phase-flow" aria-label="Estado de la evaluación">
                ${phases.map(phase => {
                    const stateClass = phase.done ? 'is-done' : (phase.optional ? 'is-optional' : 'is-pending');
                    // Marca explícita de estado: ✓ cuando la fase ocurrió de verdad, ○ cuando
                    // sigue pendiente. Hace evidente qué se hizo y qué no (antes solo cambiaba
                    // el color de fondo y costaba distinguirlo a simple vista).
                    const marker = phase.done ? iconCheck() : iconCircle();
                    const hint = phase.done
                        ? ''
                        : (phase.optional ? '<small>opcional · sin cambios</small>' : '<small>pendiente</small>');
                    return `
                        <li class="ag-phase ag-phase--${phase.key} ${stateClass}">
                            <span class="ag-phase-icon" aria-hidden="true">${phase.icon}</span>
                            <span class="ag-phase-label">${escapeHtml(phase.label)}${hint}</span>
                            <span class="ag-phase-state" aria-hidden="true">${marker}</span>
                        </li>
                    `;
                }).join('')}
            </ol>
    ...
    // Panel principal unificado (2/3): notas, evaluación por criterio editable y feedback.
    const renderDrawerEditor = (result, evaluated) => {
        if (!evaluated) {
            return '<div class="ag-empty ag-empty--compact">Todavía no hay evaluación de IA para esta entrega. '
                + 'Ejecútala con IA para poder revisar y ajustar la calificación.</div>';
        }
        const criteria = resultsCriteria();
        const finalTotal = draftFinalTotal();
        return `
            <div class="ag-review-scores">
                <div class="ag-drawer-score ag-drawer-score--ai" style="${gradeStyle(result.aitotalgrade || 0, 100)}">
                    <span>Nota IA</span>
                    <strong>${result.aitotalgrade === null ? '—' : formatNumber(result.aitotalgrade)}</strong>
                    <small>/100</small>
                </div>
                <div class="ag-drawer-score ag-drawer-score--final" data-region="final-score" style="${gradeStyle(finalTotal, 100)}">
                    <span>Nota final</span>
                    <strong>${formatNumber(finalTotal)}</strong>
                    <small>/100 · la verá el estudiante</small>
                </div>
            </div>
            <p class="ag-field-help">La nota final se calcula automáticamente a partir del nivel que elijas en cada
                criterio; no se edita directamente.</p>

            <h4>Evaluación por criterio</h4>
            <p class="ag-field-help">La retroalimentación de cada criterio es la que verá el estudiante. Parte de la
                propuesta de la IA; pulsa el lápiz para editar lo que quieras ajustar.</p>
            <div class="ag-review-criteria">
                ${state.resultDraft.criteria.map(draft => renderReviewCriterion(draft, criteria, result)).join('')}
            </div>

            <div class="ag-drawer-field">
                <label>Comentario interno del docente</label>
                <p class="ag-field-help">No visible para el estudiante. Solo para tu uso.</p>
                <textarea class="ag-textarea ag-textarea--sm" rows="3" data-action="result-draft-field"
                    data-field="studentfeedback" placeholder="Notas privadas...">${escapeHtml(state.resultDraft.studentfeedback)}</textarea>
            </div>
        `;
    };

    const renderReviewCriterion = (draft, criteria, result) => {
        const criterion = criteria.find(item => String(item.id) === String(draft.criterionid));
        if (!criterion) {
            return '';
        }
        const detail = (result.details || []).find(item => String(item.criterionid) === String(draft.criterionid)) || {};
        const levels = criterion.levels || [];
        const selected = levels.find(level => String(level.id) === String(draft.levelid));
        const score = selected ? (Number(criterion.weight) * Number(selected.percentage) / 100) : 0;
        const levelChanged = detail.levelid && String(detail.levelid) !== String(draft.levelid);
        const feedbackChanged = String(draft.finaldetail || '').trim() !== String(detail.detail || '').trim();
        const editing = Boolean(draft.editingDetail);
        return `
            <div class="ag-review-criterion" style="${gradeStyle(score, criterion.weight)}">
                <div class="ag-review-criterion-head">
                    <strong>${escapeHtml(criterion.name)}</strong>
                    <span class="ag-review-criterion-score">${formatNumber(score)} / ${formatNumber(criterion.weight)}</span>
                </div>
                <div class="ag-review-criterion-controls">
                    <label>Nivel (docente)</label>
                    <select class="ag-select ag-select--sm" data-action="review-criterion-level" data-criterion-id="${criterion.id}">
                        ${levels.map(level => `
                            <option value="${level.id}" ${String(level.id) === String(draft.levelid) ? 'selected' : ''}>${escapeHtml(level.name)} · ${formatNumber(level.percentage)}%</option>
                        `).join('')}
                    </select>
                    ${levelChanged ? '<span class="ag-tag ag-tag--amber">modificado</span>' : ''}
                </div>
                ${selected && String(selected.description || '').trim() !== ''
                    ? `<p class="ag-review-level-desc">${escapeHtml(selected.description)}</p>`
                    : ''}
                <p class="ag-review-ai">IA sugirió: <strong>${escapeHtml(detail.levelName || '—')}</strong></p>
                <div class="ag-review-feedback ${editing ? 'is-editing' : ''}">
                    <div class="ag-review-feedback-head">
                        <button type="button" class="ag-icon-btn ag-icon-btn--xs" data-action="toggle-criterion-edit"
                            data-criterion-id="${criterion.id}" aria-pressed="${editing ? 'true' : 'false'}"
                            title="${editing ? 'Bloquear edición' : 'Editar retroalimentación'}"
                            aria-label="${editing ? 'Bloquear edición' : 'Editar retroalimentación'}">
                            ${editing ? iconCheck() : iconPencil()}
                        </button>
                        <label>Retroalimentación para el estudiante</label>
                        <span class="ag-tag ag-tag--amber ag-feedback-modified" data-criterion-id="${criterion.id}"
                            ${feedbackChanged ? '' : 'hidden'}>modificado</span>
                    </div>
                    <textarea class="ag-textarea ag-textarea--xs" rows="3" data-action="review-criterion-detail"
                        data-criterion-id="${criterion.id}" ${editing ? '' : 'readonly'}
                        placeholder="Comentario de este criterio para el estudiante...">${escapeHtml(draft.finaldetail)}</textarea>
                </div>
            </div>
        `;
    };
    ...
    // Tarjeta "Actividad evaluada" en la cabecera del modal "Ver y editar": muestra de forma
    // prominente QUÉ ejercicio se está calificando (nombre + enunciado VPL), como primer
    // bloque del modal. SIEMPRE renderiza algo, para que el docente nunca vea un hueco: el
    // enunciado en un panel desplegable cuando existe, o una nota clara cuando la actividad
    // VPL no tiene descripción configurada en Moodle.
    const renderDrawerExercise = (activityName, description) => {
        const name = activityName ? String(activityName).trim() : '';
        const text = description ? String(description).trim() : '';
        const body = text === ''
            ? '<p class="ag-drawer-no-description">Esta actividad VPL no tiene un enunciado/descripción configurado en Moodle.</p>'
            : descriptionDisclosure(text, 'ag-description-toggle--wide', 'Clic para ver el enunciado del ejercicio');
        return `
            <section class="ag-drawer-exercise">
                <div class="ag-drawer-exercise-head">
                    <span class="ag-drawer-exercise-icon" aria-hidden="true">${iconBook()}</span>
                    <div class="ag-drawer-exercise-id">
                        <span class="ag-drawer-exercise-label">Actividad evaluada</span>
                        <strong>${escapeHtml(name || 'Actividad VPL')}</strong>
                    </div>
                </div>
                ${body}
            </section>
    ...
    const renderDrawerCode = result => {
        ...
        return `
            <h4>Código y resultados</h4>
            ${submission ? submissionPreview(result.submissionid) : '<div class="ag-empty ag-empty--compact">Cargando entrega...</div>'}
        `;
    };

    const resultsCriteria = () => (state.resultsState && state.resultsState.criteria)
        ? state.resultsState.criteria
        : [];

    const draftFinalTotal = () => {
        const criteria = resultsCriteria();
        let total = 0;
        state.resultDraft.criteria.forEach(draft => {
            const criterion = criteria.find(item => String(item.id) === String(draft.criterionid));
            if (!criterion) {
                return;
            }
            const level = (criterion.levels || []).find(item => String(item.id) === String(draft.levelid));
            if (level) {
                total += Number(criterion.weight) * Number(level.percentage) / 100;
            }
        });
        return Math.round(total * 100) / 100;
    };

    const buildResultDraft = result => ({
        criteria: (result.details || []).map(detail => ({
            criterionid: detail.criterionid,
            levelid: detail.finallevelid || detail.levelid || '',
            finaldetail: (detail.finaldetail !== undefined && detail.finaldetail !== '')
                ? detail.finaldetail
                : (detail.detail || ''),
            // El campo de retroalimentación por criterio nace bloqueado; el docente
            // pulsa el lápiz para editarlo (igual que el nivel se cambia en el combo).
            editingDetail: false,
        })),
        studentfeedback: result.studentfeedback || '',
    });

    // Muestra/oculta la etiqueta "modificado" del criterio mientras el docente escribe,
    // comparando contra la propuesta original de la IA (sin re-renderizar el textarea).
    const updateCriterionModifiedTag = criterionId => {
        const draft = state.resultDraft.criteria.find(
            item => String(item.criterionid) === String(criterionId));
        const details = (state.activeResult && state.activeResult.details) || [];
        const detail = details.find(item => String(item.criterionid) === String(criterionId)) || {};
        const tag = root.querySelector(`.ag-feedback-modified[data-criterion-id="${criterionId}"]`);
        if (!draft || !tag) {
            return;
        }
        tag.hidden = String(draft.finaldetail || '').trim() === String(detail.detail || '').trim();
    };
    ...
    const handleClick = async event => {
        ...
        if (action === 'view-ai-test') {
            viewAiTest(trigger.dataset.testId);
            return;
        }
        ...
        if (action === 'dismiss-reset-notice') {
            state.resultsResetNotice = '';
            renderResultsView();
            return;
        }
        ...
        if (action === 'clear-result-filters') {
        ...
        if (action === 'view-result') {
            await openResultDrawer(trigger.dataset.resultId);
            return;
        }
        ...
        if (action === 'toggle-criterion-edit') {
            const criterionId = trigger.dataset.criterionId;
            const draft = state.resultDraft.criteria.find(
                item => String(item.criterionid) === String(criterionId));
            if (draft) {
                draft.editingDetail = !draft.editingDetail;
                renderResultDrawer();
                if (draft.editingDetail) {
                    const textarea = root.querySelector(
                        `textarea[data-action="review-criterion-detail"][data-criterion-id="${criterionId}"]`);
                    if (textarea) {
                        textarea.focus();
    ...
    const handleInput = event => {
        ...
        if (action === 'review-criterion-detail') {
            const draft = state.resultDraft.criteria.find(
                item => String(item.criterionid) === String(target.dataset.criterionId));
            if (draft) {
                draft.finaldetail = target.value;
                updateCriterionModifiedTag(target.dataset.criterionId);
            }
            return;
        }
    ...
    const handleChange = async event => {
        ...
        if (action === 'review-criterion-level') {
            const draft = state.resultDraft.criteria.find(
                item => String(item.criterionid) === String(target.dataset.criterionId));
            if (draft) {
                draft.levelid = target.value;
                renderResultDrawer();
            }
            return;
        }
    ...
    const openResultsView = async() => {
        // Aviso previo si se cambió algo crítico y existen evaluaciones previas.
        if (criticalConfigChanged()) {
            const proceed = window.confirm(
                'Cambiaste la configuración de evaluación (criterios, niveles o calificaciones '
                + 'manuales de referencia). Si continúas, las calificaciones IA y las publicaciones '
                + 'previas de los estudiantes afectados se reiniciarán y deberás volver a ejecutar la '
                + 'evaluación masiva por IA.\n\n¿Deseas guardar y continuar?');
            if (!proceed) {
                return;
            }
        }
        try {
            const saveData = await saveConfiguration();
            captureConfigSnapshot();
            await loadResultsState(state.config.id);
            const reset = saveData ? Number(saveData.resultsReset || 0) : 0;
            state.resultsResetNotice = reset > 0
                ? 'Debido a los cambios en la configuración de evaluación, ' + reset + ' evaluación(es) '
                  + 'previa(s) volvieron al estado “Pendiente” y se despublicaron. Vuelve a ejecutar la '
                  + 'evaluación masiva por IA para esos estudiantes.'
                : '';
            state.view = 'results';
            renderAll();
    ...
    const openResultDrawer = async(resultId) => {
        const result = findResult(resultId);
        if (!result) {
            return;
        }
        state.activeResult = result;
        state.resultDraft = buildResultDraft(result);
        renderResultDrawer();
    ...
    const saveResultReview = async() => {
        ...
        try {
            const data = await request('save_result_review', {
                ...
                criteria: state.resultDraft.criteria.map(item => ({
                    criterionid: Number(item.criterionid),
                    levelid: Number(item.levelid),
                    finaldetail: item.finaldetail || '',
                })),
    ...
    // Muestra el desglose completo (criterios + comentario general) de una prueba IA
    // ya guardada, reutilizando el mismo panel de resultados que una prueba recién hecha.
    // No vuelve a llamar al backend: la lista de pruebas ya trae los detalles.
    const viewAiTest = testId => {
        const test = state.aiTests.find(item => String(item.id) === String(testId));
        if (!test) {
            return;
        }
        state.latestAiResult = test;
        renderAiTestCard();
        const node = root.querySelector('.ag-ai-results');
        if (node) {
    ...
    const updateTotalText = () => {
        ...
        totalNode.textContent = `Total: ${formatNumber(total)}% ${total !== 100 ? '(todos los criterios deben sumar 100%)' : ''}`;
    };
    ...
    const resultMetric = (label, value, tone) => `
        <div class="ag-metric ag-metric--${tone || 'neutral'}">
            <strong>${formatNumber(value)}</strong>
            <span>${escapeHtml(label)}</span>
        </div>
    `;

    // Inline SVG icons (no external dependencies) to mirror the Figma mockup.
    const svgIcon = paths => `<svg class="ag-svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    ...
    const iconCircle = () => svgIcon('<circle cx="12" cy="12" r="8"/>');
    ...
    // Distingue si el docente revisó el resultado o si quedó solo con la IA.
    // El estado "revisado" lo fija únicamente guardar la revisión, no publicar.
    const reviewBadge = result => {
        if (result.reviewedByTeacher) {
            return '<span class="ag-status-badge ag-status-reviewed">Revisado por docente</span>';
        }
        if (result.publicationStatus === 'published') {
            return '<span class="ag-status-badge ag-status-outline">Solo IA</span>';
        }
        return '';
    };
    ...
    const resultFeedbackPreview = result => {
        ...
        // CSS (.ag-result-feedback-preview) clamps the display to 3 lines; cap length to keep the DOM light.
        ...
        return `
            <div class="ag-code-block">
                <div class="ag-code-block-header">
                    ...
                    <div class="ag-code-block-meta">
                        ${submission.attemptNo ? `<span class="ag-chip">Intento ${escapeHtml(String(submission.attemptNo))}</span>` : ''}
