    ...
    const renderPhaseDiagram = result => {
        ...
        return `
            <ol class="ag-phase-flow" aria-label="Estado de la evaluación">
                ${phases.map(phase => `
                    <li class="ag-phase ag-phase--${phase.key} ${phase.done ? 'is-done' : (phase.optional ? 'is-optional' : 'is-pending')}">
                        <span class="ag-phase-icon" aria-hidden="true">${phase.icon}</span>
                        <span class="ag-phase-label">${escapeHtml(phase.label)}${phase.optional && !phase.done ? '<small>si aplica</small>' : ''}</span>
                    </li>
                `).join('')}
            </ol>
        `;
