define(['core/ajax'], function(Ajax) {
        ...
        <tr>
            ...
            <td class="ag-col-actions">
                <div class="ag-row-actions">
                    <button type="button" class="ag-icon-btn" data-action="view-result" data-result-id="${result.id}" title="Ver" aria-label="Ver">${iconEye()}</button>
                    <button type="button" class="ag-icon-btn" data-action="edit-result" data-result-id="${result.id}" title="Editar" aria-label="Editar">${iconPencil()}</button>
    ...
    const renderResultDrawer = () => {
        ...
        const activity = state.resultsState ? state.resultsState.activity : state.selectedActivity;
        const tab = state.resultDrawerTab || 'resumen';
        const tabs = [
            {id: 'resumen', label: 'Resumen'},
            {id: 'feedback', label: 'Feedback IA'},
            {id: 'edicion', label: 'Edición docente'},
            {id: 'codigo', label: 'Código y resultados'},
        ];
        region('main-action').innerHTML = `
            ...
            <aside class="ag-result-drawer" role="dialog" aria-modal="true" aria-label="Resultado de ${escapeAttr(result.studentName)}">
                ...
                <div class="ag-drawer-tabs" role="tablist">
                    ${tabs.map(item => `
                        <button type="button" class="ag-drawer-tab ${tab === item.id ? 'is-active' : ''}"
                            data-action="result-drawer-tab" data-tab="${item.id}" role="tab"
                            aria-selected="${tab === item.id ? 'true' : 'false'}">${escapeHtml(item.label)}</button>
                    `).join('')}
                </div>

                <div class="ag-drawer-body">
                    ${result.errordetail ? `<div class="ag-info-box ag-info-box--yellow">${escapeHtml(result.errordetail)}</div>` : ''}
                    ${tab === 'resumen' ? renderDrawerResumenTab(result, editable) : ''}
                    ${tab === 'feedback' ? renderDrawerFeedbackTab(result) : ''}
                    ${tab === 'edicion' ? renderDrawerEdicionTab(editable) : ''}
                    ${tab === 'codigo' ? renderDrawerCodigoTab(result) : ''}
                </div>
                    ...
                    <div class="ag-drawer-footer-left">
                        ...
                        ${editable ? `
                            <button type="button" class="ag-btn ag-btn--sm" data-action="save-result-review"
                                ${state.savingResult ? 'disabled' : ''}>${state.savingResult ? 'Guardando...' : 'Guardar cambios'}</button>
                        ` : `
                            <button type="button" class="ag-btn ag-btn--sm" data-action="edit-result" data-result-id="${result.id}">
                                ${iconPencil()} Editar
                            </button>
                        `}
                    </div>
                ...
                ${!isPublishable(result) && result.publicationStatus !== 'published' ? `
                    <div class="ag-drawer-hint">La evaluación IA debe estar completa para aprobar y publicar.</div>
                ` : ''}
            </aside>
        `;
    };

    const renderDrawerResumenTab = (result, editable) => `
        <section>
            <div class="ag-drawer-score" style="${gradeStyle(result.aitotalgrade || 0, 100)}">
                <span>Nota sugerida por IA</span>
            ...
            <div class="ag-drawer-field">
                ...
                <p class="ag-field-help">Esta es la nota que verá el estudiante.</p>
                <input class="ag-input ag-input--narrow" type="number" min="0" max="100" step="0.01" data-action="result-draft-field"
                    data-field="finaltotalgrade" value="${escapeAttr(state.resultDraft.finaltotalgrade)}" ${editable ? '' : 'readonly'}>
            </div>
        </section>
    `;

    const renderDrawerFeedbackTab = result => {
        const general = result.finalfeedback || resultAiFeedback(result) || '';
        return `
            <section>
                <h4>Feedback generado por IA</h4>
                ${general.trim() ? `
                    <div class="ag-feedback-box">
                        <div>${escapeHtml(general)}</div>
                    </div>
                ` : '<div class="ag-empty ag-empty--compact">No hay feedback de IA disponible.</div>'}
                ${(result.details || []).map(item => `
                    <div class="ag-feedback-box ag-feedback-box--criterion" style="${gradeStyle(item.score || 0, item.max || 100)}">
                        <label>${escapeHtml(item.criterionName)}</label>
                        <div>${escapeHtml(item.detail || 'Sin detalle.')}</div>
                    </div>
                `).join('')}
    ...
    const renderDrawerEdicionTab = editable => `
        <section>
            <div class="ag-drawer-field">
                <label>Feedback final para estudiante</label>
                <p class="ag-field-help">Este feedback será visible para el estudiante al publicar.</p>
                <textarea class="ag-textarea ag-textarea--sm" rows="8" data-action="result-draft-field"
                    data-field="finalfeedback" placeholder="Escribe tu retroalimentación personalizada aquí..."
                    ${editable ? '' : 'readonly'}>${escapeHtml(state.resultDraft.finalfeedback)}</textarea>
            </div>
            <div class="ag-drawer-field">
                <label>Comentario interno del profesor</label>
                <p class="ag-field-help">No visible al estudiante. Para uso interno del docente.</p>
                <textarea class="ag-textarea ag-textarea--sm" rows="4" data-action="result-draft-field"
                    data-field="studentfeedback" placeholder="Notas privadas sobre este estudiante o entrega..."
                    ${editable ? '' : 'readonly'}>${escapeHtml(state.resultDraft.studentfeedback)}</textarea>
            </div>
        </section>
    `;

    const renderDrawerCodigoTab = result => {
    ...
    const handleClick = async event => {
        ...
        if (action === 'result-drawer-tab') {
            state.resultDrawerTab = trigger.dataset.tab || 'resumen';
            renderResultDrawer();
            return;
        }
